...
import { meixueEssays } from '@/data/essays'

export async function generateStaticParams() {
  return meixueEssays.map((e) => ({ essay: e.slug }))
}

export async function generateMetadata({ params }: { params: { essay: string } }) {
  const essay = meixueEssays.find((e) => e.slug === params.essay)
  if (!essay) return {}
...
export default function MeixueEssayPage({ params }: { params: { essay: string } }) {
  const essay = meixueEssays.find((e) => e.slug === params.essay)
  if (!essay) notFound()

  const hasInlineImage = essay.image && essay.content.includes('【图】')

  const idx = meixueEssays.findIndex((e) => e.slug === params.essay)
  const prev = idx > 0 ? meixueEssays[idx - 1] : null
  const next = idx < meixueEssays.length - 1 ? meixueEssays[idx + 1] : null

  const EssayNav = () => (
    <nav className="flex items-center justify-between py-4 border-t border-amber-200/40 dark:border-gray-800/60">
      <div className="flex-1">
        {prev && (
          <Link href={`/luzexi/meixue/${prev.slug}`} className="group flex items-center gap-2 text-sm font-sans nav-link">
      ...
      <Link href="/luzexi/meixue" className="font-sans text-xs text-gray-400 dark:text-gray-500 hover:text-accent dark:hover:text-amber-400 transition-colors px-4 tracking-wide">
        美学篇目录
      </Link>
      <div className="flex-1 text-right">
        {next && (
          <Link href={`/luzexi/meixue/${next.slug}`} className="group inline-flex items-center gap-2 text-sm font-sans nav-link">
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 sm:px-6 py-10 w-full">
        ...
        <div className="text-center mt-2 mb-2">
          <ViewCounter page={`meixue-${essay.slug}`} />
        </div>
        ...
        <Comments page={`meixue-${essay.slug}`} />
      </main>
